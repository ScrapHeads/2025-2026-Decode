...
//        double x = RobotState.getInstance().getEstimatedPose().getX();
        ...
        double targetAngle = angle - RobotState.getInstance().getEstimatedPose().getRotation().getDegrees();

        double targetPos = targetAngle * TurretRotate.TICKS_PER_DEGREE;

        turretRotate.setTargetPos(targetPos);

        TelemetryPacket packet = new TelemetryPacket();
        packet.put("Launch angle", targetAngle);
